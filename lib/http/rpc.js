...
RPC.prototype.createRawTransaction = async function createRawTransaction(args, help) {
  ...
  const replacable = valid.bool(3);
  const strFloData = valid.str(4);
  ...
  // Add in floData string to transaction
  tx.setFloData(strFloData)
...
RPC.prototype.signRawTransaction = async function signRawTransaction(args, help) {
  ...
  let sig = await tx.signAsync(keys, type, this.workers);
